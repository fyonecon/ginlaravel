...
// CreateCacheMQ 创建缓存延迟执行队列事件
// 调用本函数建议用多线程（互斥锁）
func CreateCacheMQ(ctx *gin.Context, key string, id1 int64, id2 int64, callFunc CallFunc) int64 {
	// 消息事件和消息参数写入Redis
	...
	v, _ := json.Marshal(_v)
	err := RDB.LPush(ctx, k, v).Err()
	if err != nil {
		fmt.Println(err)
		Log("CreateCacheMQ缓存写入失败", "")
		return 0
	}
	return 1
}

// DoCacheMQ 读取Redis队列数据，并生成队列的目标回调事件
// 将一次性执行完所有队列MQ事件（可定时调用但不能单次调用）
func DoCacheMQ(ctx *gin.Context) {
	// 按序读出数据
	_k := "gl_cache_mq"
	// 获取表中元素的个数
	_len, _ := RDB.LLen(ctx, _k).Result()
	_val, _err := RDB.LRange(ctx, _k, 0, _len-1).Result()
	if _err != nil {
		fmt.Println(_err)
		return
	}
