...
// ExampleWeb 面向模版。访问：你的域名/web/空间命名/具体方法
func ExampleWeb(route *gin.Engine) { //

	// web分组路由
	web := route.Group("/web/", Middleware.HttpCorsWeb)
	{
		web.Any("", Tpl.Tpl1)
		example := web.Group("/example/", Middleware.HttpLimiter(3), Example.VerifyExample)
		{
			example.Any("tpl", Tpl.Tpl2)
			example.Any("websocket", func(ctx *gin.Context) {
				ctx.HTML(200, "pages/websocket/tpl-websocket.html", gin.H{
					"title": "websocket html test",
				})
			})
		}
