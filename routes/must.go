...
import (
	...
	//_ "./docs" // 执行「swag init」生成的docs文件夹路径 _引包表示只执行init函数
	"net/http"
)
...
func Must(route *gin.Engine) {
	...
	route.GET("/swagger/*any", Middleware.HttpCorsWeb, Middleware.HttpLimiter(2), ginSwagger.WrapHandler(swaggerFiles.Handler, url))
